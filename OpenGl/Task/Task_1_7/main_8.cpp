...
vector<class::Triangle*> polygons_1q;
vector<class::Triangle*> polygons_2q;
vector<class::Triangle*> polygons_3q;
vector<class::Triangle*> polygons_4q;

Line lx(-1.f, 0.f, 0.f, 1.f, 0.f, 0);
Line ly(0.f, 1.f, 0.f, 0.f, -1.f, 0);
GLuint vao1[3], vao2[3], vao3[3], vao4[3], vaol[2];
GLuint vaom[12];
GLuint vbo[4];
...
GLvoid drawScene() //--- 콜백 함수: 그리기 콜백 함수 {
{
	...
	for (size_t i = 0; i < polygons_1q.size(); i++)
	{
		glBindVertexArray(vao1[i]);
		polygons_1q[i]->Draw_Polygon();
	}
	for (size_t i = 0; i < polygons_2q.size(); i++)
	{
		glBindVertexArray(vao2[i]);
		polygons_2q[i]->Draw_Polygon();
	}
	for (size_t i = 0; i < polygons_3q.size(); i++)
	{
		glBindVertexArray(vao3[i]);
		polygons_3q[i]->Draw_Polygon();
	}
	for (size_t i = 0; i < polygons_4q.size(); i++)
	{
		glBindVertexArray(vao4[i]);
		polygons_4q[i]->Draw_Polygon();
	}
	...
	case GLUT_RIGHT_BUTTON: // 사각형 내부 - 색상 랜덤, 사각형 외부 - 배경색 랜덤
	{
		if (state == GLUT_DOWN)
		{
			...
			case mode_Triangle:
			{
				...
				case 1:
					if (polygons_1q.size() < 3)
						polygons_1q.emplace_back(temp);
					break;
				case 2:
					if (polygons_2q.size() < 3)
						polygons_2q.emplace_back(temp);
					break;
				case 3:
					if (polygons_3q.size() < 3)
						polygons_3q.emplace_back(temp);
					break;
				case 4:
					if (polygons_4q.size() < 3)
						polygons_4q.emplace_back(temp);
					break;
				default:
					break;
				}
...
void InitBuffer()
{
	...
	for (size_t i = 0; i < polygons_1q.size(); i++)
	{
		...
		polygons_1q[i]->init_buffer_polygon(vao1, vbo);
	}
	glGenVertexArrays(10, vao2); //--- VAO 를 지정하고 할당하기
	for (size_t i = 0; i < polygons_2q.size(); i++)
	{
		glBindVertexArray(vao2[i]); //--- VAO를 바인드하기
		polygons_2q[i]->init_buffer_polygon(vao2, vbo);
	}
	glGenVertexArrays(10, vao3); //--- VAO 를 지정하고 할당하기
	for (size_t i = 0; i < polygons_3q.size(); i++)
	{
		glBindVertexArray(vao3[i]); //--- VAO를 바인드하기
		polygons_3q[i]->init_buffer_polygon(vao3, vbo);
	}
	glGenVertexArrays(10, vao4); //--- VAO 를 지정하고 할당하기
	for (size_t i = 0; i < polygons_4q.size(); i++)
	{
		glBindVertexArray(vao4[i]); //--- VAO를 바인드하기
		polygons_4q[i]->init_buffer_polygon(vao4, vbo);
	}
