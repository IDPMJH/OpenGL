...
#include "global.h"
#include "Polygon_13.h"
#include "Line_13.h"
#include "component_13.h"
...
void main(int argc, char** argv) //--- 윈도우 출력하고 콜백함수 설정 { //--- 윈도우 생성하기
{
	...
	int i = glutCreateWindow("Task_13"); // 윈도우 생성 (윈도우 이름)
...
GLvoid drawScene() //--- 콜백 함수: 그리기 콜백 함수 {
{
	...
	modelT= glm::rotate(modelT, radians(10.f), vec3(1.f, -1.f, 0.f)); // x축, -y축 기준으로 회전
...
GLvoid Keyboard(unsigned char key, int x, int y)
{
	...
	case 'c':
		...
		model._face_on_1 = Random_Number<int>(1,6);
		model._face_on_2 = Random_Number<int>(1, 6);
