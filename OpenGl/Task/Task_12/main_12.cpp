...
void main(int argc, char** argv) //--- 윈도우 출력하고 콜백함수 설정 { //--- 윈도우 생성하기
{
	...
	float ran_x = Random_Number<int>(-90, 90) / 100.f;
	float ran_y = Random_Number<int>(-90, 90) / 100.f;
	float offset = 0.1f;


	class::Dot* dot = new class::Dot(ran_x, ran_y,0.f);

	ran_x = Random_Number<int>(-90, 90) / 100.f;
	ran_y = Random_Number<int>(-90, 90) / 100.f;
	class::Line* line = new Line(ran_x, ran_y,offset);

	ran_x = Random_Number<int>(-90, 90) / 100.f;
	ran_y = Random_Number<int>(-90, 90) / 100.f;
	class::Polygon* Tri = new Triangle(ran_x,ran_y, offset,0);
	Tri->_move = move_End;
	Tri->Change_Color(0.f, 0.f, 1.f);


	ran_x = Random_Number<int>(-90, 90) / 100.f;
	ran_y = Random_Number<int>(-90, 90) / 100.f;
	class::Polygon* Rect = new Rect_p(ran_x, ran_y, offset);
	Rect->Change_Color(1.f, 0.f, 0.f);

	ran_x = Random_Number<int>(-90, 90) / 100.f;
	ran_y = Random_Number<int>(-90, 90) / 100.f;
	class::Polygon* Penta = new Pentagon(ran_x,ran_y,offset);
	Penta->Change_Color(0.f, 1.f, 0.f);

	
	Polygons.emplace_back(dot);
	Polygons.emplace_back(line);
	Polygons.emplace_back(Tri);
	Polygons.emplace_back(Rect);
	Polygons.emplace_back(Penta);
...
GLvoid Mouse_Down_Move(int x, int y)
{
	...
	glm::vec2 vpos(x_offset, y_offset);

	for (auto& rc : Polygons)
	{
		if (rc->_clicked == true)
			rc->Update(vpos);
	}
...
GLvoid Mouse(int button, int state, int x, int y)
{
	...
	case GLUT_LEFT_BUTTON: // 사각형 내부 - 색상 랜덤, 사각형 외부 - 배경색 랜덤
	{
		if (state == GLUT_DOWN)
		{
			...
			for (auto& a : Polygons)
			{
				a->Left_Click(xpos, ypos);
			}
		}
		else if (state == GLUT_UP)
		{
			for (auto rc = Polygons.begin(); rc != Polygons.end(); ++rc)
			{
				if ((*rc)->_clicked == true)
					(*rc)->Merge_Check(Polygons);
			}
			// 꼭지점 개수에 따른 생성
			int vertex_count = 0;
			int merge_count = 0.f;
...
GLvoid Keyboard(unsigned char key, int x, int y)
{
	...
	if (key == 'a' && _draws_of_all == false)
	{
		Polygons.clear();
		class::Polygon* lx = new Line(-1.f, 0.f, 0.f, 1.f, 0.f, 0);
		class::Polygon* ly = new Line(0.f, 1.f, 0.f, 0.f, -1.f, 0);

		class::Polygon* LinetoTri = new Triangle(-0.8f, 0.2f, 0.f, -0.5f, 0.8f, 0.f, -0.5f, 0.799f, 0.f);
		LinetoTri->Change_Color(0.f, 0.f, 1.f);

		class::Polygon* TritoRect = new Rect_p(0.5f, 0.8f, 0.2f, 0.2f, 0.8f, 0.2f);
		TritoRect->Change_Color(1.f, 0.f, 0.f);

		class::Polygon* RecttoPenta = new Pentagon(-0.8f, -0.4f, -0.8f, -0.8f, -0.2f, -0.8f, -0.2f, -0.4f);
		RecttoPenta->Change_Color(0.f, 1.f, 0.f);

		class::Polygon* PentatoLine = new Pentagon(0.5f, -0.2f, 0.2f, -0.4f, 0.3f, -0.8f, 0.7f, -0.8f, 0.8f, -0.4f);
		PentatoLine->Change_Color(0.f, 1.f, 1.f);

		Polygons.emplace_back(lx);
		Polygons.emplace_back(ly);
		Polygons.emplace_back(LinetoTri);
		Polygons.emplace_back(TritoRect);
		Polygons.emplace_back(RecttoPenta);
		Polygons.emplace_back(PentatoLine);
		_draws_of_all = true;
	}
	else if (key == 'l')
	{
		_draws_of_all = false;
		Polygons.clear();
		class::Polygon* LinetoTri = new Triangle(-0.8f+0.5f, 0.2f - 0.5f, 0.f, -0.5f + 0.5f, 0.8f - 0.5f, 0.f, -0.5f + 0.5f, 0.799f - 0.5f, 0.f);
		LinetoTri->Change_Color(0.f, 0.f, 1.f);
		Polygons.emplace_back(LinetoTri);
	}
	else if (key == 't')
	{
		_draws_of_all = false;
		Polygons.clear();
		class::Polygon* TritoRect = new Rect_p(0.5f-0.5f, 0.8f - 0.5f, 0.2f - 0.5f, 0.2f - 0.5f, 0.8f - 0.5f, 0.2f - 0.5f);
		TritoRect->Change_Color(1.f, 0.f, 0.f);
		Polygons.emplace_back(TritoRect);
	...
	else if (key == 'r')
	{
		...
		_draws_of_all = false;
		Polygons.clear();
		class::Polygon* RecttoPenta = new Pentagon(-0.8f + 0.5f, -0.4f + 0.5f, -0.8f + 0.5f, -0.8f + 0.5f, -0.2f + 0.5f, -0.8f + 0.5f, -0.2f + 0.5f, -0.4f + 0.5f);
		RecttoPenta->Change_Color(0.f, 1.f, 0.f);
		Polygons.emplace_back(RecttoPenta);
	...
	else if (key == 'p')
	{
		...
		class::Polygon* PentatoLine = new Pentagon(0.0f, 0.3f, -0.3f, 0.1f, -0.2f, -0.3f, 0.2f, -0.3f, 0.3f, 0.1f);
		PentatoLine->Change_Color(0.f, 1.f, 1.f);
		Polygons.emplace_back(PentatoLine);
